...
// Script to update ArrayList.deinit(self.allocator) calls to ArrayList.deinit() for Zig 0.16 compatibility
pub fn main() !void {
    ...
    const files_to_update = [_][]const u8{
        "src/audio/audio.zig",
        "src/physics/rigid_body.zig",
        "src/physics/physics_engine.zig",
...
fn updateFile(file_path: []const u8, allocator: std.mem.Allocator) !void {
    ...
    // Replace .deinit(self.allocator) with .deinit()
    var updated_content = try std.mem.replaceOwned(u8, allocator, content, ".deinit(self.allocator)", ".deinit()");

    // Also handle cases with different allocator names (like self.child_allocator)
