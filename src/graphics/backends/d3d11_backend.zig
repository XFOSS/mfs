...
pub const D3D11Backend = struct {
    ...
    fn createSwapChainImpl(impl: *anyopaque, desc: *const interface.SwapChainDesc) interface.GraphicsBackendError!void {
        ...
        swap_chain_desc.BufferDesc.Format = convertTextureFormat(desc.format);
    ...
    fn createTextureImpl(impl: *anyopaque, texture: *types.Texture, data: ?[]const u8) interface.GraphicsBackendError!void {
        ...
        desc.Format = convertTextureFormat(texture.format);
        ...
        if (data) |texture_data| {
            subresource_data = c.D3D11_SUBRESOURCE_DATA{
                ...
                .SysMemPitch = texture.width * getBytesPerPixel(texture.format),
                .SysMemSlicePitch = 0,
            };
    ...
    // Utility functions
    fn convertTextureFormat(format: types.TextureFormat) c.DXGI_FORMAT {
        return switch (format) {
            .rgba8 => c.DXGI_FORMAT_R8G8B8A8_UNORM,
            .rgb8 => c.DXGI_FORMAT_R8G8B8A8_UNORM, // D3D11 doesn't have RGB8, use RGBA8
            .bgra8 => c.DXGI_FORMAT_B8G8R8A8_UNORM,
            .r8 => c.DXGI_FORMAT_R8_UNORM,
    ...
    fn getBytesPerPixel(format: types.TextureFormat) u32 {
        return switch (format) {
