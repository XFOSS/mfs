...
pub const Dispatch = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !*Dispatch {
        ...
        disp.vkGetInstanceProcAddr = @ptrCast(GetInstanceFn)(lib.lookup("vkGetInstanceProcAddr") orelse return DispatchError.SymbolNotFound);
        disp.vkGetDeviceProcAddr = @ptrCast(GetDeviceFn)(lib.lookup("vkGetDeviceProcAddr") orelse return DispatchError.SymbolNotFound);

        // Load a couple of essential functions
        disp.vkCreateInstance = @ptrCast(typeof(disp.vkCreateInstance))(disp.vkGetInstanceProcAddr(null, "vkCreateInstance") orelse return DispatchError.SymbolNotFound);
        disp.vkDestroyInstance = @ptrCast(typeof(disp.vkDestroyInstance))(disp.vkGetInstanceProcAddr(null, "vkDestroyInstance") orelse return DispatchError.SymbolNotFound);
    ...
    pub fn loadInstance(self: *Dispatch, instance: vk.VkInstance) !void {
        ...
        self.vkEnumeratePhysicalDevices = @ptrCast(EnumeratePhysicalDevicesFn)(self.vkGetInstanceProcAddr(instance, "vkEnumeratePhysicalDevices") orelse return DispatchError.SymbolNotFound);
        // Load CreateDevice
        self.vkCreateDevice = @ptrCast(CreateDeviceFn)(self.vkGetInstanceProcAddr(instance, "vkCreateDevice") orelse return DispatchError.SymbolNotFound);
        // Load DestroySurfaceKHR
        self.vkDestroySurfaceKHR = @ptrCast(DestroySurfaceKHRFn)(self.vkGetInstanceProcAddr(instance, "vkDestroySurfaceKHR") orelse return DispatchError.SymbolNotFound);
    }

    /// Load device-level functions after vkCreateDevice
    pub fn loadDevice(self: *Dispatch, device: vk.VkDevice) !void {
        // Load GetDeviceQueue
        self.vkGetDeviceQueue = @ptrCast(GetDeviceQueueFn)(self.vkGetDeviceProcAddr(device, "vkGetDeviceQueue") orelse return DispatchError.SymbolNotFound);
    }
};
