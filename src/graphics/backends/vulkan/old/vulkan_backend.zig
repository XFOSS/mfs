...
pub const VulkanBackend = struct {
    ...
    const SwapchainData = struct {
        ...
        pub fn init(allocator: std.mem.Allocator) SwapchainData {
            return SwapchainData{
                .handle = undefined,
                .images = std.ArrayList(vkmod.Image).initCapacity(allocator, 4) catch unreachable,
                .image_views = std.ArrayList(vkmod.ImageView).initCapacity(allocator, 4) catch unreachable,
    ...
    const SyncObjects = struct {
        ...
        pub fn init(allocator: std.mem.Allocator) SyncObjects {
            return SyncObjects{
                .image_available = std.ArrayList(vkmod.Semaphore).initCapacity(allocator, 4) catch unreachable,
                .render_finished = std.ArrayList(vkmod.Semaphore).initCapacity(allocator, 4) catch unreachable,
                .in_flight_fences = std.ArrayList(vkmod.Fence).initCapacity(allocator, 4) catch unreachable,
            };
        }
    ...
    const MemoryManager = struct {
        ...
        const MemoryPool = struct {
            ...
            pub fn init(allocator: std.mem.Allocator, device: vkmod.Device, memory_type_index: u32, flags: vkmod.MemoryPropertyFlags) !MemoryPool {
                ...
                var blocks = std.ArrayList(MemoryBlock).initCapacity(allocator, 4) catch unreachable;
                try blocks.append(.{
                    .offset = 0,
                    .size = POOL_SIZE,
                    .is_free = true,
    ...
    pub fn init(allocator: std.mem.Allocator, config: interface.BackendConfig) !*Self {
        ...
        self.* = Self{
            ...
            .command_buffers = std.ArrayList(vkmod.CommandBuffer).initCapacity(allocator, 4) catch unreachable,
            .current_frame = 0,
