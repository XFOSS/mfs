...
// Export the backend creation function
pub fn createBackend(allocator: std.mem.Allocator) !interface.GraphicsBackend {
    var backend = try D3D12Backend.init(allocator);

    return interface.GraphicsBackend{
        .allocator = allocator,
        .backend_type = .d3d12,
        .initialized = backend.initialized,
        .vtable = .{
            .deinit = @ptrCast(&D3D12Backend.deinit),
            .create_swap_chain = @ptrCast(&D3D12Backend.createSwapChain),
            .resize_swap_chain = @ptrCast(&D3D12Backend.resizeSwapChain),
            .present = @ptrCast(&D3D12Backend.present),
            .get_current_back_buffer = @ptrCast(&D3D12Backend.getCurrentBackBuffer),
            .create_texture = @ptrCast(&createTextureStub),
            .create_buffer = @ptrCast(&createBufferStub),
            .create_shader = @ptrCast(&createShaderStub),
            .create_pipeline = @ptrCast(&createPipelineStub),
            .create_render_target = @ptrCast(&createRenderTargetStub),
            .update_buffer = @ptrCast(&updateBufferStub),
            .update_texture = @ptrCast(&updateTextureStub),
            .destroy_texture = @ptrCast(&destroyTextureStub),
            .destroy_buffer = @ptrCast(&destroyBufferStub),
            .destroy_shader = @ptrCast(&destroyShaderStub),
            .destroy_render_target = @ptrCast(&destroyRenderTargetStub),
            .create_command_buffer = @ptrCast(&createCommandBufferStub),
            .begin_command_buffer = @ptrCast(&beginCommandBufferStub),
            .end_command_buffer = @ptrCast(&endCommandBufferStub),
            .submit_command_buffer = @ptrCast(&submitCommandBufferStub),
            .begin_render_pass = @ptrCast(&beginRenderPassStub),
            .end_render_pass = @ptrCast(&endRenderPassStub),
            .set_viewport = @ptrCast(&setViewportStub),
            .set_scissor = @ptrCast(&setScissorStub),
            .bind_pipeline = @ptrCast(&bindPipelineStub),
            .bind_vertex_buffer = @ptrCast(&bindVertexBufferStub),
            .bind_index_buffer = @ptrCast(&bindIndexBufferStub),
            .bind_texture = @ptrCast(&bindTextureStub),
            .bind_uniform_buffer = @ptrCast(&bindUniformBufferStub),
            .draw = @ptrCast(&drawStub),
            .draw_indexed = @ptrCast(&drawIndexedStub),
            .dispatch = @ptrCast(&dispatchStub),
            .copy_buffer = @ptrCast(&copyBufferStub),
            .copy_texture = @ptrCast(&copyTextureStub),
            .copy_buffer_to_texture = @ptrCast(&copyBufferToTextureStub),
            .copy_texture_to_buffer = @ptrCast(&copyTextureToBufferStub),
            .resource_barrier = @ptrCast(&resourceBarrierStub),
            .get_backend_info = @ptrCast(&D3D12Backend.getBackendInfo),
            .set_debug_name = @ptrCast(&setDebugNameStub),
            .begin_debug_group = @ptrCast(&beginDebugGroupStub),
            .end_debug_group = @ptrCast(&endDebugGroupStub),
        },
        .impl_data = @intFromPtr(&backend),
    };
}
