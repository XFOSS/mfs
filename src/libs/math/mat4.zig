...
pub fn Mat4(comptime T: type) type {
    return struct {
        ...
        /// Matrix-vector multiplication for Vec3 (convenience method)
        ...
        pub fn fromTransform(translate: @import("vec3.zig").Vec3(T), rotation: @import("vec3.zig").Vec3(T), scale_vec: @import("vec3.zig").Vec3(T)) Self {
            // Create rotation matrix from euler angles
            const rx = rotationX(rotation.x);
            const ry = rotationY(rotation.y);
            const rz = rotationZ(rotation.z);
            const rotation_matrix = rz.mul(ry).mul(rx);

            return transform(translate, rotation_matrix, scale_vec);
        }

        /// Create transformation matrix from translation, quaternion rotation, and scale
        pub fn fromTransformQuaternion(translate: @import("vec3.zig").Vec3(T), rotation: @import("vector.zig").Quaternion, scale_vec: @import("vec3.zig").Vec3(T)) Self {
            const rotation_matrix = rotation.toMatrix();
            return transform(translate, rotation_matrix, scale_vec);
        }

        /// Create transformation matrix from rotation quaternion and translation
        pub fn fromRotationTranslation(rotation: @import("vector.zig").Quaternion, translate: @import("vec3.zig").Vec3(T)) Self {
            const rotation_matrix = rotation.toMatrix();
            var result = rotation_matrix;
        ...
        /// Create transformation matrix from translation, rotation matrix, and scale
