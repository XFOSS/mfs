...
test "vector normalize" {
    ...
    try expectApproxEqAbs(normalized.length(), 1.0, 0.0001);
}
...
test "matrix multiplication" {
    ...
    const m = math.Mat4.translation(math.Vec3.new(1.0, 2.0, 3.0));
...
test "matrix translation" {
    ...
    const ty = -3.0;
    const tz = 7.0;
    const t = math.Mat4.translation(math.Vec3.new(tx, ty, tz));
...
test "matrix scaling" {
    ...
    const sy = 3.0;
    const sz = 4.0;
    const s = math.Mat4.scaling(math.Vec3.new(sx, sy, sz));
    ...
    const scaled = s.transformVector(v);
...
test "matrix rotation" {
    ...
    const rotated = r.transformVector(v);
...
test "matrix inversion" {
    ...
    const m = math.Mat4.translation(math.Vec3.new(1.0, 2.0, 3.0))
        .mul(math.Mat4.scaling(math.Vec3.new(2.0, 2.0, 2.0)));
    
    // Invert it
    const inv = m.invert() catch unreachable;
...
test "quaternion to/from euler angles" {
    ...
    const euler = math.Vec3.new(
        std.math.pi / 4.0,  // X - pitch - 45 degrees
